...
import { auth, db, doc, updateDoc, onSnapshot } from '../utils/firebase';
import { Picker } from '@react-native-picker/picker';
import { MaterialIcons } from '@expo/vector-icons';
import { SvgUri } from 'react-native-svg'; // For ability icons
...
export default function EditCharacterScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (user && characterId) {
      ...
      const unsubscribe = onSnapshot(characterRef, (docSnap) => {
        if (docSnap.exists()) {
          const charData = docSnap.data();
          setName(charData.name || '');
          setPhoto(charData.photo || '');
          setRace(charData.race || 'Human');
          setCharClass(charData.class || 'Barbarian');
          setLevel(charData.level?.toString() || '1');
          setBackground(charData.background || 'Acolyte');
          setAlignment(charData.alignment || 'Lawful Good');
          setStrength(charData.abilityScores?.strength?.toString() || '10');
          setDexterity(charData.abilityScores?.dexterity?.toString() || '10');
          setConstitution(charData.abilityScores?.constitution?.toString() || '10');
          setIntelligence(charData.abilityScores?.intelligence?.toString() || '10');
          setWisdom(charData.abilityScores?.wisdom?.toString() || '10');
          setCharisma(charData.abilityScores?.charisma?.toString() || '10');
          setCurrency(charData.currency?.toString() || '0');
        }
      });
      return () => unsubscribe();
    }
  ...
  const saveCharacter = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', user.uid, 'characters', characterId), {
        name: name.trim(),
        photo: photo || '',
        race,
        class: charClass,
  ...
  return (
    <DismissKeyboard>
      <View style={styles.container}>
        <BackButton onPress={() => navigation.navigate('Character', { characterId })} />
        <ScrollView style={styles.characterContainer}>
          <Card>
            <Text style={styles.sectionHeader}>Edit Character</Text>
            ...
            <CustomButton title="Save Character" onPress={saveCharacter} />
